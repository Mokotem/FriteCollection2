...
public class Button : Text
{
    ...
    public void Update(bool mouseHold, bool isMouseOn, bool active)
    {
        bgColor = new Color(0.9f, 0.9f, 0.9f);
        t = 2;
        if (active && this.active)
        {
            if (active && rect.Contains(parent.GetMousePos())
    ...
    public override void Draw(in SpriteBatch batch)
    {
        if (active)
        {
            ...
            batch.DrawRectangle(rect.ToRectangleF(), new Color(0f, 0.5f, 1f), t, layer);
            base.Draw(in batch);
        }
    }
}
